...
<section class="teachers">
   <h1 class="heading" style = "margin-top: 20px; margin-bottom: 30px;">Your Order Records !</h1>
</section>

    @if(Session()->has('order_cancelled_from_customer'))
        <div class="msgHehe" id="msg1">
            {{session()->get('order_cancelled_from_customer')}}
    ...
    <div class="all_product_image_container" style = "padding-left: 20px;">
                ...
                <div style = "width: 100%;">
                    <div class="product-card" data-product-id="{{ $each_order->id }}" style = "width: 30%; padding-bottom: 20px;">
                        ...
                        <div class="product-details" style = "position : absolute; margin-left: 380px; margin-top: -280px;">
                            ...
                            <div class="buy_cart_container">
                                @if($each_order->is_order_accepted == 1)
                                    <button class="cart_btn_unique disable_cart_btn add-to-cart-button">Cancel Order</button>
                                @elseif(($each_order->is_order_cancelled == 1))
                                    <button class="cart_btn_unique disable_cart_btn add-to-cart-button">Cancel Order</button>
                                @else
                                    <a onclick="return confirm('Are You Sure To Cancel The Order With Name : {{$each_order->product->product_name}} From Your Order List !')" href = "{{ route('cancel_order_from_customer', $each_order->id) }}" ><button class="cart_btn_unique add-to-cart-button">Cancel Order</button></a>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="main">
                        ...
                        <ul>
                            ...
                            @if($each_order->is_order_placed == 1 && $each_order->is_order_cancelled == null)
                                <li>
                                    ...
                                    <!-- <br> -->
                                    <!-- <p class="text">{{ $each_order->order_placed_date }}</p> -->
                                </li>
                            @elseif($each_order->is_order_cancelled == 1)
                                <li>
                                    <i class="icon uil uil-clipboard-notes"></i>
                                    <div class="progress one {{ $each_order->is_order_cancelled == 1 ? 'active_cancel' : '' }}">
                                        <p>1</p>
                            ...
                            @if($each_order->is_order_accepted == null)
                                <li>
                                    <i class="icon uil uil-cloud-check"></i>
                                    <div class="progress two {{ $each_order->is_order_accepted == 1 ? 'active' : '' }}">
                                        <p>2</p>
                            ...
                            @if($each_order->is_order_accepted == -1)
                                <li>
                                    <i class="icon uil uil-cloud-check"></i>
                                    <div class="progress two {{ $each_order->is_order_accepted == -1 ? 'active_cancel' : '' }}">
                                        <p>2</p>
                                        <i class="uil uil-times"></i>
                                    </div>
                                    <p class="text">Order Accepted</p>
        ...
        @else
            <div class="container">
                ...
                <h2>No any product orderd yet</h2>
            </div>
        @endif
    </div>
