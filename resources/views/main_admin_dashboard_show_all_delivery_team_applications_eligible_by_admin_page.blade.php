...
@section('body')
    <div class="page-wrapper">
        <div class="content container-fluid">
            ...
            @if(count($all_delivery_team_applications_eligible_by_admin) > 0)
            <div class="center">
                <h2 id="main-heading">All Delivery Team Applications Eligible From Admin</h2>
                ...
                <div class="table_container">
                    <table id="cat_table">
                        <tr id="header_row">
                            ...
                            <td>Proof of eligibility Test Document</td>
                            <td>Employee ID who uploaded proof of test document</td>
                            ...
                            <td>To Unconfirm to Applicant</td>
                        </tr>
                        ...
                        @foreach($all_delivery_team_applications_eligible_by_admin as $each_delivery_team_application)
                            <tr id="all_rows">
                                <td>{{ $srNo++ }}</td>  
                                ...
                                <td>{{ $each_delivery_team_application->employee_id_first }}</td>

                                <td>{{ $each_delivery_team_application->is_confirmed_from_admin_first }}</td>
                                <td>{{ $each_delivery_team_application->admin_id_first }}</td>

                                <td>{{ $each_delivery_team_application->is_contacted }}</td>
                                <td>{{ $each_delivery_team_application->employee_id_who_contacted }}</td>

                                <td>
                                    <a href="{{ route('delivery_team_application_proof_of_test_document_viewer', ['id' => $each_delivery_team_application->id]) }}" target="_blank" id="document_btn">Click For Document</a>
                                </td>
                                <td>{{ $each_delivery_team_application->employee_id_who_upload_proof_of_test_document }}</td>

                                <td>{{ $each_delivery_team_application->is_confirmed_from_employee_second }}</td>
                                <td>{{ $each_delivery_team_application->employee_id_second }}</td>

                                <td>{{ $each_delivery_team_application->is_confirmed_from_admin_second }}</td>
                                <td>{{ $each_delivery_team_application->admin_id_second }}</td>

                                <td>{{ $each_delivery_team_application->confirmation_code ?? '-----------' }}</td>
                                <td>{{ $each_delivery_team_application->is_joined }}</td>
                                <!-- <td>{{ $each_delivery_team_application->username ?? '-----------' }}</td>
                                <td>{{ $each_delivery_team_application->confirm_password ?? '-----------' }}</td> -->

                                <td>{{ $each_delivery_team_application->created_at }}</td>
                                <td>{{ $each_delivery_team_application->updated_at }}</td>
                                <td><a onclick="return confirm('Are you sure to Unconfirm selected Delivery Applicant Application !')" href="{{ route('unconfirming_delivery_team_application_from_admin_second_step', $each_delivery_team_application->id) }}" id="delete_btn">Unconfirm</a></td> 
                            </tr>
                        @endforeach
            ...
            @else
                <script>
                    window.location.href = '{{ route('main_admin_dashboard_nothing_to_show_in_delivery_team_applications_eligible_by_admin_page') }}';
                </script>
            @endif
        </div>
    </div>
