...
<head>
   ...
   <title>Seller Accepted Orders</title>
...
<section class="teachers">
   <h1 class="heading" style = "margin-top: 20px; margin-bottom: 30px;">Your Accepted Order Records !</h1>
</section>

    @if(Session()->has('order_acceptance_cancelled'))
        <div class="msg" id="msg1">
            {{session()->get('order_acceptance_cancelled')}}
    ...
    @if(Session()->has('order_cancelled'))
        <div class="msgTry" id="msg1">
            {{session()->get('order_cancelled')}}
            <button type="button" onclick = "document.getElementById('msg1').style.display = 'none';" id="popup-btn" >X</button>
        </div>
    @endif

    @if(Session()->has('order_marked_as_ready'))
        <div class="msgTry" id="msg1">
            {{session()->get('order_marked_as_ready')}}
            <button type="button" onclick = "document.getElementById('msg1').style.display = 'none';" id="popup-btn" >X</button>
        </div>
    @endif
    ...
    <div class="all_product_image_container" style = "padding-left: 20px;">
                ...
                <div style = "width: 100%;">
                    <div class="product-card" data-product-id="{{ $each_order->id }}" style = "width: 30%; padding-bottom: 20px;">
                        ...
                        <div class="product-details" style = "position : absolute; margin-left: 380px; margin-top: -280px;">
                            ...
                            <div class="buy_cart_container" style = "margin-top: 20px; width: 110%;">
                                <a onclick="return confirm('Are You Sure To Mark As Ready The Order With Name : {{$each_order->product->product_name}} From Your Order List !')" href="{{ route('mark_order_as_ready_from_seller', $each_order->id) }}"><button class="cart_btn add-to-cart-button">Mark As Ready</button></a>
                                <a onclick="return confirm('Are You Sure To Cancel acceptance of The Order With Name : {{$each_order->product->product_name}} From Your Order List !')" href="{{ route('Cancel_order_Acceptance_from_seller', $each_order->id) }}"><button class="cart_btn add-to-cart-button">Cancel Acceptance</button></a>
                                <a onclick="return confirm('Are You Sure To Cancel The Order With Name : {{$each_order->product->product_name}} From Your Order List !')" href="{{ route('cancel_order_from_seller', $each_order->id) }}"><button class="cart_btn add-to-cart-button">Cancel Order</button></a>
                            </div>
                        </div>
                    </div>
        ...
        @else
            <div class="container">
                ...
                <h2>No any order accepted yet</h2>
            </div>
        @endif
    </div>
