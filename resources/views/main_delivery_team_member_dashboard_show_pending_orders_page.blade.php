...
<head>
   ...
   <title>Delivery Team Member Pending Orders</title>
...
<section class="teachers">
   <h1 class="heading" style = "margin-top: 20px; margin-bottom: 30px;">Your Pending Delivery Records !</h1>
</section>

    @if(Session()->has('delivery_acceptance_cancelled'))
        <div class="msg" id="msg1">
            {{session()->get('delivery_acceptance_cancelled')}}
            <button type="button" onclick = "document.getElementById('msg1').style.display = 'none';" id="popup-btn" >X</button>
        </div>
    @endif

    @if(Session()->has('marked_as_shipped'))
        <div class="msg_shipped" id="msg1">
            {{session()->get('marked_as_shipped')}}
            <button type="button" onclick = "document.getElementById('msg1').style.display = 'none';" id="popup-btn" >X</button>
        </div>
    @endif
    ...
    <div class="all_product_image_container" style = "padding-left: 20px;">
        ...
        @if(count($all_accepted_orders) > 0)
            @foreach($all_accepted_orders as $each_order)
                    ...
                    <div class="product-card" data-product-id="{{ $each_order->id }}" style = "width: 30%; padding-bottom: 20px;">
                        ...
                        <div class="product-details" style = "position : absolute; margin-left: 380px; margin-top: -280px;">
                            ...
                            <div class="buy_cart_container" style = "margin-top: 20px;">
                                <a onclick="return confirm('Are You Sure To Mark As Shipped The Order With Name : {{$each_order->product->product_name}} From accepted order List !')" href="{{ route('order_mark_as_shipped_from_delivery_team_member', $each_order->id) }}"><button class="cart_btn add-to-cart-button">Mark As Shipped</button></a>
                                <a onclick="return confirm('Are You Sure To cancel the delivery acceptance With Name : {{$each_order->product->product_name}} From Your accepted order List !')" href="{{ route('cancel_delivery_from_delivery_team_member', $each_order->id) }}"><button class="cart_btn add-to-cart-button">Cancel Acceptance</button></a>
                            </div>
                        </div>
                    </div>
        ...
        @else
            <div class="container">
                ...
                <h2>No any order pending yet</h2>
            </div>
        @endif
    </div>
