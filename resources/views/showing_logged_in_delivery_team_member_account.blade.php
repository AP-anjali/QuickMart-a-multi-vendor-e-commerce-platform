...
<head>
   ...
   <title>{{ $delivery_team_member_data->name }} | Profile</title>
...
<script>
    ...
    function checkFormChanges(){
        let name = "{{ $delivery_team_member_data->name }}";
        let email = "{{ $delivery_team_member_data->email }}";
        let phone_no = "{{ $delivery_team_member_data->phone_no }}";
        let address = "{{ $delivery_team_member_data->address }}";

        let gender = "{{ $delivery_team_member_data->gender }}";
        let date_of_birth = "{{ $delivery_team_member_data->date_of_birth }}";
        let photo = "{{ $delivery_team_member_data->photo }}";
        let Aadharcard = "{{ $delivery_team_member_data->Aadharcard }}";
        let vehicle_rc_book = "{{ $delivery_team_member_data->vehicle_rc_book }}";
        let vehicle_puc = "{{ $delivery_team_member_data->vehicle_puc }}";
        let vehicle_licence = "{{ $delivery_team_member_data->vehicle_licence }}";
        let vehicle_insurance = "{{ $delivery_team_member_data->vehicle_insurance }}";


        let original_profile_pic = "{{ $delivery_team_member_data->profile_pic }}";

        let username = "{{ $delivery_team_member_data->username }}";
        let confirm_password = "{{ $delivery_team_member_data->confirm_password }}";

        let bank_name = "{{ $delivery_team_member_data->bank_name }}";
        let bank_branch = "{{ $delivery_team_member_data->bank_branch }}";
        let bank_ifsc_code = "{{ $delivery_team_member_data->bank_ifsc_code }}";
        let bank_micr_code = "{{ $delivery_team_member_data->bank_micr_code }}";
        let account_holder_name = "{{ $delivery_team_member_data->account_holder_name }}";
        let account_number = "{{ $delivery_team_member_data->account_number }}";
        let account_type = "{{ $delivery_team_member_data->account_type }}";
        let original_bank_document = "{{ $delivery_team_member_data->proof_of_bank_account_ownership }}";
        ...
        let input_vehicle_rc_book = document.getElementById('vehicle_rc_book').files[0];
        let input_vehicle_puc = document.getElementById('vehicle_puc').files[0];
        let input_vehicle_licence = document.getElementById('vehicle_licence').files[0];
        let input_vehicle_insurance = document.getElementById('vehicle_insurance').files[0];
        ...
        && (!input_vehicle_rc_book || input_vehicle_rc_book.name === vehicle_rc_book)
        && (!input_vehicle_puc || input_vehicle_puc.name === vehicle_puc)
        && (!input_vehicle_licence || input_vehicle_licence.name === vehicle_licence)
        && (!input_vehicle_insurance || input_vehicle_insurance.name === vehicle_insurance)
...
<section class="contact"> <!-- customer_account -->
    ...
    @if(Session()->has('delivery_team_member_record_to_update'))
        <div class="msg" id="msg1">
            {{session()->get('delivery_team_member_record_to_update')}}
    ...
    <div class="customer_account_main_container">
        ...
        <form action="{{ route('updating_delivery_team_member_user_record_from_delivery_team_member_page_form_submission', $delivery_team_member_data->id) }}" method = "post" enctype="multipart/form-data" id="accountDetailsForm">
            @csrf
            ...
            <div class="customer_account_image_slot_container">
                <img src="{{ asset('storage/' . $delivery_team_member_data->profile_pic) }}" alt="User Profile Pic">
            ...
            <div class="users_info_container">
                ...
                <div class="button-container">
                    <div class="text-fields name">
                        ...
                        <input type="text" name="name" id="name" value = "{{ $delivery_team_member_data->name }}" placeholder="Your full name" required oninput="checkFormChanges()">
                    </div>
                    <div class="text-fields email">
                        <label for="email"><i class='bx bx-envelope bx-md' style='color:#6487cb'></i></label>
                        <input type="text" name="email" id="email" value = "{{ $delivery_team_member_data->email }}" placeholder="Your Email Address" required oninput="checkFormChanges()">
                    </div>
                </div>
                ...
                <div class="button-container">
                    <div class="text-fields phone_no">
                        ...
                        <input type="text" name="phone_no" id="phone_no" value = "{{ $delivery_team_member_data->phone_no }}" placeholder="Phone number with +91" required oninput="checkFormChanges()">
                    </div>
                    <div class="text-fields address123">
                        <label for="address"><i class='bx bxl-periscope bx-md' style='color:#6487cb' ></i></label>
                        <input type="text" name="address" id="address" value = "{{ $delivery_team_member_data->address }}" placeholder="Your Business address" required oninput="checkFormChanges()">
                    </div>
                </div>
                ...
                <div class="button-container">
                    <div class="text-fields gender">
                        ...
                        <input type="text" name="gender" id="gender" value = "{{ $delivery_team_member_data->gender }}" placeholder="Your Gender" required oninput="checkFormChanges()">
                    </div>
                    <div class="text-fields date_of_birth">
                        <label for="date_of_birth"><i class='bx bx-calendar bx-md' style='color:#6487cb' ></i></label>
                        <input type="date" name="date_of_birth" id="date_of_birth" value = "{{ $delivery_team_member_data->date_of_birth }}" placeholder="Your Date Of Birth" required oninput="checkFormChanges()" style = "padding-right: 55px;">
                    </div>
                </div>
                    ...
                    <div id="secured_data_that_need_user_verification">
                        ...
                        <div class="button-container" style = "margin-left: 10px;">
                            <div class = "customer_account_current_bank_document_btn" style = "margin: 0 60px;">
                                <a href = "{{ route('showing_photo_to_delivery_team_member_for_account', $delivery_team_member_data->id) }}" target = "_blank">CURRENT PHOTO</a>
                            </div>

                            <div class = "customer_account_current_bank_document_btn" style = "margin: 0 60px;">
                                <a href = "{{ route('showing_aadharcard_to_delivery_team_member_for_account', $delivery_team_member_data->id) }}" target = "_blank">CURRENT AADHARCARD</a>
                            </div>
                        </div>
                        ...
                        <div class="button-container">
                            <div class="file_uploader">
                                <label for="vehicle_rc_book"><i  class='bx bxs-folder-open bx-md' style='color:#6487cb'></i></label>
                                <input type="file" name="vehicle_rc_book" id="vehicle_rc_book" onchange="checkFormChanges();">
                            </div>
                            <div class="file_uploader_heading">Vehicle RC Book</div>

                            <div class="file_uploader" style = "margin-left: -113px;">
                                <label for="vehicle_puc"><i  class='bx bxs-folder-open bx-md' style='color:#6487cb'></i></label>
                                <input type="file" name="vehicle_puc" id="vehicle_puc" onchange="checkFormChanges();">
                            </div>
                            <div class="file_uploader_heading">Vehicle PUC</div>
                        </div>

                        <div class="button-container" style = "margin-left: 10px;">
                            <div class = "customer_account_current_bank_document_btn" style = "margin: 0 60px;">
                                <a href = "{{ route('showing_rc_book_to_delivery_team_member_for_account', $delivery_team_member_data->id) }}" target = "_blank">CURRENT RC BOOK</a>
                            </div>

                            <div class = "customer_account_current_bank_document_btn" style = "margin: 0 60px;">
                                <a href = "{{ route('showing_puc_to_delivery_team_member_for_account', $delivery_team_member_data->id) }}" target = "_blank">CURRENT PUC</a>
                            </div>
                        </div>
                        ...
                        <div class="button-container" style = "margin-top: 40px;">
                            <div class="file_uploader">
                                <label for="vehicle_licence"><i  class='bx bxs-folder-open bx-md' style='color:#6487cb'></i></label>
                                <input type="file" name="vehicle_licence" id="vehicle_licence" onchange="checkFormChanges();">
                            </div>
                            <div class="file_uploader_heading">Vehicle Licence</div>

                            <div class="file_uploader" style = "margin-left: -103px;">
                                <label for="vehicle_insurance"><i  class='bx bxs-folder-open bx-md' style='color:#6487cb'></i></label>
                                <input type="file" name="vehicle_insurance" id="vehicle_insurance" onchange="checkFormChanges();">
                            </div>
                            <div class="file_uploader_heading">Vehicle Insurance</div>
                        </div>

                        <div class="button-container" style = "margin-left: 10px;">
                            <div class = "customer_account_current_bank_document_btn" style = "margin: 0 60px;">
                                <a href = "{{ route('showing_licence_to_delivery_team_member_for_account', $delivery_team_member_data->id) }}" target = "_blank">CURRENT LICENCE</a>
                            </div>

                            <div class = "customer_account_current_bank_document_btn" style = "margin: 0 60px;">
                                <a href = "{{ route('showing_INSURANCE_to_delivery_team_member_for_account', $delivery_team_member_data->id) }}" target = "_blank">CURRENT INSURANCE</a>
                            </div>
                        </div>
                        <!-- ----------------------- -->
                        ...
                        <div class="button-container">
                            <div class="text-fields bank_name" >
                                ...
                                <input type="text" name="bank_name" id="bank_name" value = "{{ $delivery_team_member_data->bank_name }}" required oninput="checkFormChanges()">
                            </div>
                            <div class="text-fields bank_branch" >
                                <label for="bank_branch"><i class='bx bxs-doughnut-chart bx-md' style='color:#6487cb'></i></label>
                                <input type="text" name="bank_branch" id="bank_branch" value = "{{ $delivery_team_member_data->bank_branch }}" required oninput="checkFormChanges()">
                            </div>
                        </div>
                        <div class="button-container">
                            <div class="text-fields bank_ifsc_code" >
                                <label for="bank_ifsc_code"><i class='bx bxs-bank bx-md' style='color:#6487cb'></i></label>
                                <input type="text" name="bank_ifsc_code" id="bank_ifsc_code" value="{{ $delivery_team_member_data->bank_ifsc_code }}" placeholder="Bank Accont IFSC Code" required oninput="checkFormChanges()">
                            </div>
                            <div class="text-fields bank_micr_code" >
                                ...
                                <input type="text" name="bank_micr_code" id="bank_micr_code" value="{{ $delivery_team_member_data->bank_micr_code }}" placeholder="Bank Accont MICR Code" required oninput="checkFormChanges()">
                            </div>
                        </div>
                        <div class="button-container">
                            <div class="text-fields account_holder_name">
                                ...
                                <input type="text" name="account_holder_name" id="account_holder_name" value = "{{ $delivery_team_member_data->account_holder_name }}" required oninput="checkFormChanges()">
                            </div>
                            <div class="text-fields account_number">
                                ...
                                <input type="text" name="account_number" id="account_number" value = "{{ $delivery_team_member_data->account_number }}" required oninput="checkFormChanges()">
                            </div>
                        </div>
                        <div class="button-container">
                            <div class="text-fields account_type">
                                ...
                                <select name="account_type" id="my_list4" required onchange="checkFormChanges()">
                                    <option value="{{ $delivery_team_member_data->account_type }}" selected>{{ $delivery_team_member_data->account_type }}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</option>
                        ...
                        <div class="button-container" style = "margin-top: -30px;">
                            <div class = "customer_account_current_bank_document_btn">
                                <a href = "{{ route('showing_proof_of_bank_account_ownership_to_delivery_team_member_for_account', $delivery_team_member_data->id) }}" target = "_blank">CURRENT PROOF OF BANK ACCOUNT OWNERSHIP</a>
                            </div>
                        </div>
                        ...
                        <!-- ------------------------------------ -->
                            ...
                            <div class="button-container">
                                <div class="text-fields username">
                                    ...
                                    <input type="text" name="username" id="username" value = "{{ $delivery_team_member_data->username }}" placeholder="Create Own Username" required oninput="checkFormChanges()">
                                </div>

                                <div class="text-fields confirm_password">
                                    <label for="confirm_password"><i class='bx bxs-key bx-md' style='color:#6487cb' ></i></label>
                                    <input type="password" name="confirm_password" id="confirm_password" value = "{{ $delivery_team_member_data->confirm_password }}" placeholder="Enter Password" required oninput="checkFormChanges()">
                            ...
                            <div class="button-container customer_account_creation_info_slot" style = "margin-top: 60px;">
                                ...
                                <div class = "customer_account_creation_date">
                                    <h2><?php echo date("d/m/Y, h:i:s A", strtotime($delivery_team_member_data->created_at)); ?></h2>
                                </div>
                            </div>
        ...
        <div class="users_info_container">
            ...
            <div class="verify_user_for_secured_data" style="{{ Session::has('verification_code_correct') ? 'display: none;' : '' }}">
                ...
                <button class = "user_email_verification_btn" onclick = "goToRouteToSentVerificationCode({{ $delivery_team_member_data->id }})" style="{{ Session::has('mail_has_been_sent_to_user') ? 'pointer-events: none; opacity: 0.5;' : '' }}">EMAIL VERIFICATION</button>
            </div>
            ...
            @if(Session::has('mail_has_been_sent_to_user'))
                <div id="email_verification_form_that_will_be_dispalyed_when_an_email_has_been_sent">
                    <form action="{{ route('verifying_delivery_team_member_email_verification_code_form_submission', $delivery_team_member_data->id) }}" method = "post">
                        @csrf
                        <div class="email_text_box_container">
                            <div class="email_v_text">We have sent a verification code on <span class = "users_mail">"{{ $delivery_team_member_data->email }}"</span></div>
                            <div class="button-container">
            ...
            <div class="change_reset_btn_slot">
                <div class="button-container">
                    <div class = "customer_account_deletion_btn disabled_due_to_checkbox_data" id = "id_for_enable_desable_delete_account_btn">
                        <a onclick = "return confirm('Are You Sure To Delete Your Account !');" href = "{{ route('deleting_delivery_team_member_account_from_delivery_team_member', $delivery_team_member_data->id) }}" id="deleteButton">DELETE ACCOUNT</a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    {
        var redirecting_route = "/sending_email_for_verify_delivery_team_member_user/" + userID;
